...
func (conn *Connection) constructDatagram(opts *Options, datagram []byte) (addr netip.AddrPort, datagramSize int, addToSendQueue bool) {
	conn.mu.Lock()
	defer conn.mu.Unlock()
	addr = conn.addr
	var err error
...
func (conn *Connection) constructDatagramLocked(opts *Options, datagram []byte) (int, bool, error) {
...
func (conn *Connection) constructDatagramAcks(opts *Options, datagramLeft []byte) (int, error) {
...
func (conn *Connection) constructDatagramAlert(opts *Options, datagramLeft []byte, alert record.Alert) (int, error) {
	sendSymmetric := conn.keys.SendSymmetric
	sendEpoch := conn.keys.SendEpoch
	sendNextSeq := &conn.keys.SendNextSeq
	if conn.hctx != nil && sendEpoch < 2 {
