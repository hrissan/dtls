...
type Stats interface {
	...
	MustNotBeFragmented(kind string, message string, addr netip.AddrPort, header format.HandshakeMsgFragmentHeader)
	MustBeEncrypted(kind string, message string, addr netip.AddrPort, header format.HandshakeMsgFragmentHeader)
	MustNotBeEncrypted(kind string, message string, addr netip.AddrPort, header format.HandshakeMsgFragmentHeader)

	// logic layer
	ErrorServerReceivedServerHello(addr netip.AddrPort)
	ErrorClientReceivedClientHello(addr netip.AddrPort)
	ErrorClientHelloUnsupportedParams(handshakeHdr format.HandshakeMsgFragmentHeader, msg format.MsgClientHello, addr netip.AddrPort, err error)
	ErrorServerHelloUnsupportedParams(handshakeHdr format.HandshakeMsgFragmentHeader, msg format.MsgServerHello, addr netip.AddrPort, err error)
	ClientHelloMessage(handshakeHdr format.HandshakeMsgFragmentHeader, msg format.MsgClientHello, addr netip.AddrPort)
	ServerHelloMessage(handshakeHdr format.HandshakeMsgFragmentHeader, msg format.MsgServerHello, addr netip.AddrPort)
...
func (s *StatsLog) MustNotBeFragmented(kind string, message string, addr netip.AddrPort, header format.HandshakeMsgFragmentHeader) {
	if s.level.Load() < 0 {
		return
	}
...
func (s *StatsLog) MustBeEncrypted(kind string, message string, addr netip.AddrPort, header format.HandshakeMsgFragmentHeader) {
	if s.level.Load() < 0 {
		return
	}
...
func (s *StatsLog) MustNotBeEncrypted(kind string, message string, addr netip.AddrPort, header format.HandshakeMsgFragmentHeader) {
	if s.level.Load() < 0 {
		return
	}
...
func (s *StatsLog) ErrorClientHelloUnsupportedParams(handshakeHdr format.HandshakeMsgFragmentHeader, msg format.MsgClientHello, addr netip.AddrPort, err error) {
...
func (s *StatsLog) ErrorServerHelloUnsupportedParams(handshakeHdr format.HandshakeMsgFragmentHeader, msg format.MsgServerHello, addr netip.AddrPort, err error) {
	if !s.printMessages.Load() {
		return
	}
	log.Printf("tinydtls: message %s header=%+v has unsupported params addr=%v: %+v: %v", msg.MessageName(), handshakeHdr, addr, msg, err)
...
func (s *StatsLog) ClientHelloMessage(handshakeHdr format.HandshakeMsgFragmentHeader, msg format.MsgClientHello, addr netip.AddrPort) {
	if !s.printMessages.Load() {
		return
	}
...
func (s *StatsLog) ServerHelloMessage(handshakeHdr format.HandshakeMsgFragmentHeader, msg format.MsgServerHello, addr netip.AddrPort) {
	if !s.printMessages.Load() {
		return
	}
	log.Printf("tinydtls: message %s header=%+v addr=%v: %+v", msg.MessageName(), handshakeHdr, addr, msg)
